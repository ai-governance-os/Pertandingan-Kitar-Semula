...
state = d.addStarEvent(state, event(20));
assert.equal(quota().remaining, 0);
const ledgerBeforeReset = JSON.stringify(state.starLedger);
const studentBeforeReset = d.studentStarBalance(state, 'test-student');
state = d.resetTeacherMonthlyQuota(state, ['TEACHER_A'], 'ADMIN');
assert.equal(quota().used, 0);
assert.equal(quota().remaining, 100, 'Manual reset restores configured allowance');
assert.equal(quota().totalIssued, 100, 'Actual monthly issuance is retained');
assert.equal(JSON.stringify(state.starLedger), ledgerBeforeReset);
assert.equal(d.studentStarBalance(state, 'test-student'), studentBeforeReset);
assert.equal(quota('TEACHER_B').limit, 0, 'Individual reset leaves others unchanged');
const afterResetAward = event(35);
state = d.addStarEvent(state, afterResetAward);
assert.equal(quota().used, 35, 'Same-millisecond awards after reset count');
state = d.removeStarEvent(state, afterResetAward.id);
assert.equal(quota().used, 35, 'Deleting post-reset award does not refill quota');
state = d.resetTeacherMonthlyQuota(state, ['TEACHER_A', 'TEACHER_B'], 'ADMIN');
assert.equal(quota().remaining, 100, 'Repeated reset restores full allowance, not cumulative credit');
assert.equal(quota('TEACHER_B').remaining, 0, 'Reset does not unpause a zero-limit account');
assert.equal(state.teacherQuotaResets.length, 3);
assert.equal(state.teacherQuotaResets[0].adminId, 'ADMIN');
state = d.load();
assert.equal(quota().used, 0, 'Manual resets survive reload');
state = d.resetSeason(state);
assert.equal(quota().used, 0, 'Season reset preserves manual-reset baseline');
assert.equal(state.teacherQuotaResets.length, 3, 'Audit trail retained');
state = d.addStarEvent(state, event(10));
...
assert.equal(quota().lastResetAt, null, 'Prior-month manual resets do not affect new month');
console.log('PASS: individual/all/repeated manual resets, audit trail, unchanged student balance, zero-limit account, persistence and monthly rollover.');
