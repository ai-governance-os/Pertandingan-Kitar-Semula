...
for(const f of ['data.js','hornbeetle-rig.jsx','pet-body-rigs.js','pet-living-profiles.js']){
...
s.BeetleRig=s.window.BeetleRig;const {EcoData,PetLivingRig,PetBodyRigs,BeetleRig}=s.window;
assert.equal(Object.keys(PetBodyRigs.limbs).length,50);let forms=0,wings=0;const signatures=new Set();
for(const {id} of EcoData.PET_SPECIES){
 ...
 for(let stage=1;stage<6;stage++){
  const rig=PetLivingRig.get(id,stage);assert(rig?.feet.length>=2,id+stage);forms++;
...
assert.equal(forms,250);assert(signatures.size>=200,'Species and stages do not all use one motion');
...
console.log(`PASS: all 50 species / ${forms} bodies, ${wings} winged forms, visible limb/wing travel, bounded poses, reduced motion, cached rigs.`);
