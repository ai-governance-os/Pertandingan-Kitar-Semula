...
function CinematicSharedPark({ report, teams, teamFilter, setTeamFilter, onPick, onPetInteract, onYakultOpen }) {
  ...
  function interact(row,button) {
    ...
    setReaction(window.PetOwnerVoice ? PetOwnerVoice.line(row) : `${row.name} · 弹跳打招呼`);
  ...
  const playPetAccent = useCallback((row) => {
    ...
    start().then((ok) => {
      if (ok && engineRef.current && window.PetOwnerVoice?.isCurrent(request)) {
        if(speaking&&!useCreatureCall)engineRef.current.duck(window.PetCharacterVoice.recording(row).duration);
        else playCreature();
      }
    });
  }, [start]);
